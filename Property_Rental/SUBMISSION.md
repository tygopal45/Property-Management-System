...
- **Live application:** TODO — the image and the blueprint are built and verified; the hosting
  accounts are not created yet. See *Deploying it* below for exactly what is done and what is left.
...
Python image, and `render.yaml` is the blueprint. I ran the image against an **empty** database to
prove the whole production path rather than assume it: Alembic migrated from nothing, the seed ran,
the service came up, and **the 51-clause requirement audit passed against the container**. Restarting
it does not re-seed — `seed.py --if-empty` checks first and says so when it declines, because
seeding blindly on every deploy would either duplicate the demo data or fail on a unique constraint
and take the service down with it.

**What is left, and it needs accounts rather than code.**

1. Create a free **Aiven MySQL** service and copy its connection string.
2. Create a **Render** service from this repository — the blueprint is at `render.yaml` in the
   repository root, so *New → Blueprint* picks it up.
3. Set `DATABASE_URL` in Render to the Aiven string. Aiven requires TLS, so it needs
   `?ssl_ca=/etc/ssl/certs/ca-certificates.crt` on the end.
4. Set `CORS_ORIGINS` to the Render URL once assigned. `JWT_SECRET` is generated by Render and
   never seen by anyone, which is the point.
5. Record the live URL above, and note here how long a cold start takes.
