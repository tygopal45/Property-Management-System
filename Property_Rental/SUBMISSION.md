...
> yet — the image and the blueprint are built and verified, and creating the hosting account is
> the one step left.** The goal checklist below says Done only where there is working, tested code
...
- **Live application:** TODO — the image and the blueprint are built and verified against an empty
  database; the Render account is not created yet. `render.yaml` declares the service *and* its
  database, so deploying is *New → Blueprint* and one environment variable. See *Deploying it*.
...
**One service, not two.** The API process serves the built browser app from the same origin. That
is a deliberate call rather than a shortcut: two origins would mean CORS with credentials and a
session cookie downgraded from `SameSite=Lax` to `SameSite=None`, which is the exact protection
`config.py` refuses to give up when it rejects a wildcard origin. It also means one URL and one
cold start instead of two.

**One platform, and no secret typed anywhere.** `render.yaml` declares both the web service and a
...
**What is left, and it needs an account rather than code.**

1. Create a **Render** service from this repository via *New → Blueprint* — `render.yaml` at the
   repository root defines the service and the database together.
2. Set `CORS_ORIGINS` to the Render URL once assigned. It is belt and braces on one origin; a
   wildcard is rejected by `config.py` on purpose.
3. Record the live URL above, and note here how long a cold start takes.
...
above runs the whole thing locally in under a minute. I chose the single-platform setup knowing this;
the alternative was a second provider and a hand-carried password.
...
| Hosting | Render — one service, one database | The API process serves the built browser app, so there is one origin, one URL and one cold start, and the session cookie stays `SameSite=Lax`. `render.yaml` declares both, so no password is ever pasted |
